...
import { GoogleGenerativeAI, SchemaType } from "@google/generative-ai"; // Pastikan SchemaType masuk
...
function formatForWhatsApp(text: string) {
  if (!text) return "";
  let formatted = text;
  // Ubah Bintang Ganda (Gemini Bold) jadi Bintang Tunggal (WA Bold)
  formatted = formatted.replace(/\*\*(.*?)\*\*/g, '*$1*');
  // Ubah Heading (### Judul) jadi Bold WA (*Judul*)
  ...
  try {
    ...
    if (body.object === "whatsapp_business_account") {
      ...
      if (messages && messages.length > 0 && messages[0].type === "text") {
        ...
        // Panggil ekstraksi pintar di background (Tidak usah dikasih 'await' biar webhook WA cepat selesai)
...
async function runLeadExtractionBackground(client: any, senderPhone: string, userName: string, userMsg: string, aiReply: string, history: any[]) {
  try {
    // 1. STANDARISASI NOMOR (Menghindari beda format)
    ...
    // 2. GEMINI STRUCTURED OUTPUT (Anti gagal parse JSON & Halusinasi Data)
    ...
    // 3. SMART MERGE: AMBIL DATA LAMA DARI DATABASE SEBELUM DITIMPA
    ...
    // Logika Emas: Kalau AI menangkap data baru, pakai data baru. 
    // Tapi kalau AI bilang "null" (lupa), kita pakai data lama dari database.
    ...
    // 4. THE MAGIC UPSERT (TIDAK AKAN PERNAH DUPLIKAT KARENA SQL CONSTRAINT)
    const { error: upsertError } = await supabase
      .from("leads")
      .upsert({
        id: oldLead?.id, // Akan update baris ini jika ID sudah ada. Jika undefined, akan buat baris baru.
        client_id: client.id,
        ...
        is_bot_active: oldLead ? oldLead.is_bot_active : true // Jangan nyalakan bot kalau emang lagi dimatiin sama admin
      }, {
        onConflict: 'client_id, customer_phone', // Memerintahkan Supabase untuk me-replace baris yang nomor HP dan ID kliennya sama.
        ignoreDuplicates: false // Izinkan penimpaan (Update)
      });

    if (upsertError) {
      console.error("❌ Gagal Smart Upsert WA Lead:", upsertError.message);
    } else {
      console.log(`✅ Smart Upsert WA Sukses: ${phoneNumber}`);
    }

  } catch (e) {
    console.error("⚠️ Background Extraction Error:", e);
  }
}
