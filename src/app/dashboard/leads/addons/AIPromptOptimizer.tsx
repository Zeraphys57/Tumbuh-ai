...
export default function AIPromptOptimizer({ currentPrompt, setPrompt }: AIPromptOptimizerProps) {
  ...
  const runOptimizer = async () => {
    ...
    try {
      ...
      // PROTEKSI: Jika hasil optimasi dari AI ternyata kelewatan limit
      ...
      setLocalError("AI Optimizer sedang sibuk. Coba sebentar lagi ya!");
    } finally {
  ...
  return (
    <div className="relative overflow-hidden bg-white rounded-[2.5rem] border border-blue-200 shadow-lg transition-all duration-500 hover:shadow-[0_20px_50px_-15px_rgba(59,130,246,0.15)] mb-6 h-fit flex flex-col group z-10">
      ...
      <div className="relative z-10 p-6 flex justify-between items-start border-b border-slate-100 bg-white/80 backdrop-blur-sm">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-blue-500 text-[10px] font-bold uppercase tracking-widest italic">Core Addon</span>
          </div>
        ...
        >
          {isOptimizing ? (
            <><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Working...</>
          ) : (
            <>✨ Enhance Prompt</>
          )}
        </button>
      ...
      <div className="p-6 flex-1 flex flex-col relative z-10 bg-slate-50/50">
        {localError && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-xl text-red-500 text-[9px] font-bold tracking-widest uppercase text-center animate-in fade-in zoom-in-95">
            ⚠️ {localError}
          </div>
        )}
        ...
        {isOptimizing ? (
           <div className="flex-1 flex flex-col items-center justify-center text-slate-400 h-32">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] italic animate-pulse text-blue-600">Re-engineering Persona...</p>
           </div>
        ) : result ? (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
           ...
           <div className="flex-1 flex flex-col items-center justify-center text-slate-400 h-32 text-center">
              <svg className="w-8 h-8 mb-2 opacity-30 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
              <p className="text-[10px] font-bold italic text-slate-500 text-center px-4 leading-relaxed">
                Tekan tombol di atas untuk menyulap instruksi Anda menjadi <span className="text-blue-500">Master Prompt</span> profesional.
              </p>
           </div>
        )}
      </div>
