...
export default function LeadsTable({ 
  ...
  const renderChatLog = (chatText: string, lead: any) => {
    ...
    return messages.map((msg, i) => {
      ...
      // [FIX]: Menambahkan max-w-full dan break-words pada system badge agar tidak meluber
      if (msg.role === 'system') return <div key={i} className={`flex justify-center opacity-70 ${marginTop}`}><span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] bg-white px-4 py-1.5 rounded-full border border-slate-200 shadow-sm max-w-full break-words text-center">{msg.text || 'Sesi Baru'}</span></div>;
      
      // [FIX]: Menambahkan w-fit, max-w-[90%], break-words dan whitespace-pre-wrap pada bubble chat
      if (msg.role === 'user') {
        return (
          <div key={i} className={`flex justify-end w-full ${marginTop}`} id={isTarget && matchedKw ? `target-${lead.id}-${formatKw(matchedKw)}` : undefined}>
            <div className={`py-3 px-5 rounded-2xl text-xs font-medium leading-relaxed w-fit max-w-[90%] md:max-w-[80%] break-words whitespace-pre-wrap transition-all duration-700 ${isSameAsPrev ? 'rounded-tr-md' : 'rounded-tr-sm'} ${isTarget ? 'bg-gradient-to-br from-yellow-400 to-amber-600 text-white shadow-[0_0_20px_rgba(251,191,36,0.6)] ring-4 ring-yellow-400/30 scale-[1.02]' : 'bg-indigo-600 text-white shadow-sm'}`} dangerouslySetInnerHTML={{ __html: formatHtml(msg.text) }} />
          </div>
        );
      }
      
      return <div key={i} className={`flex justify-start w-full ${marginTop}`}><div className={`bg-white text-slate-700 py-3 px-5 rounded-2xl border border-slate-200 shadow-sm text-xs font-medium leading-relaxed w-fit max-w-[90%] md:max-w-[85%] break-words whitespace-pre-wrap ${isSameAsPrev ? 'rounded-tl-md' : 'rounded-tl-sm'}`} dangerouslySetInnerHTML={{ __html: formatHtml(msg.text) }} /></div>;
    });
  };
  ...
  return (
    <>
      {/* [BARU 🌟]: Efek Blur Background kalau lagi di-maximize */}
      {isMaximized && (
        <div 
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[998] transition-opacity animate-[fadeIn_0.3s_ease-out]" 
          onClick={() => setIsMaximized(false)}
        ></div>
      )}

      {/* Container dinamis: Berubah jadi modal raksasa kalau isMaximized = true */}
      <div className={`transition-all duration-500 ease-in-out ${
        isMaximized 
          ? "fixed inset-2 md:inset-6 lg:inset-10 z-[999] bg-white rounded-[2.5rem] shadow-2xl flex flex-col border border-slate-200 overflow-hidden" 
          : "bg-white rounded-[2.5rem] shadow-2xl border border-white overflow-hidden relative max-w-full"
      }`}>
        ...
        <div className="p-6 md:p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col gap-5 flex-shrink-0">
          ...
          <div className="flex flex-col xl:flex-row items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center justify-center xl:justify-end gap-3 w-full">
              ...
              <input 
                ...
                className="flex-1 sm:flex-none appearance-none bg-white border border-slate-200 text-slate-700 text-xs font-bold px-4 py-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer shadow-sm w-full min-w-[120px]"
              />
              
              <div className="flex gap-2 w-full sm:w-auto">
                 <button onClick={handleDownloadCSV} className="flex-1 sm:flex-none justify-center bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm active:scale-95 flex items-center gap-1.5">
                   CSV
                 </button>
                 <button onClick={handleDownloadPDF} className="flex-1 sm:flex-none justify-center bg-slate-900 hover:bg-indigo-600 text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md active:scale-95 flex items-center gap-1.5">
                   PDF
                 </button>
                 {/* [BARU 🌟]: TOMBOL TOGGLE MAXIMIZE (Dihide di HP dengan class hidden sm:flex) */}
                 <button 
        ...
        {/* [BARU 🌟]: Wrapper tabel overflow-x-auto agar kalau dipaksa sempit, user bisa geser tabel ke kanan-kiri */}
                  ...
                  <tr className="hover:bg-indigo-50/20 transition-all duration-300">
                    <td className="px-6 py-5 w-[30%] max-w-[250px] align-top">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="font-black text-slate-800 text-sm leading-none mb-1.5 flex items-center gap-2 truncate">
                            {lead.customer_name || "Customer"}
                          </p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono truncate">{lead.customer_phone}</p>
                        </div>
                      </div>
                    </td>

                    {/* [FIX]: Menghapus whitespace-nowrap pada badges dan membiarkan flex-wrap agar nyusun rapi */}
                    <td className="px-6 py-5 w-[40%] max-w-[300px] align-top">
                      <div className="flex flex-wrap gap-2 w-full">
                        {lead.computedBadges.map((badge: any, idx: number) => (
                           <span key={idx} className="bg-slate-100 text-slate-600 border border-slate-200 text-[10px] px-2.5 py-1 rounded-md font-bold shadow-sm inline-block break-words max-w-full">
                    ...
                    <td className="px-6 py-5 w-[15%] align-top">
                       <select 
                         ...
                         className={`text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border shadow-sm outline-none cursor-pointer appearance-none ${getStatusColor(lead.agent_status)} w-full text-center truncate`}
                       >
                    ...
                    <td className="px-6 py-5 w-[15%] text-right align-top">
                      <div className="flex items-center justify-end gap-2">
                         <button onClick={() => setExpandedRowId(expandedRowId === lead.id ? null : lead.id)} className="h-9 w-9 bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 hover:border-indigo-300 rounded-xl shadow-sm flex items-center justify-center transition-all flex-shrink-0" title="Lihat Detail">
                         ...
                         {lead.platform !== 'instagram' && lead.platform !== 'gmail' && (
                            <a href={`https://wa.me/${lead.customer_phone}`} target="_blank" rel="noopener noreferrer" className={`h-9 w-9 ${buttonColor} ${buttonHover} text-white rounded-xl shadow-md flex items-center justify-center transition-all hover:-translate-y-0.5 flex-shrink-0`} title="Balas via WA">
                      ...
                      <td colSpan={4} className="p-0">
                        <div className="flex flex-col lg:flex-row gap-6 p-6 md:p-8 animate-[fadeIn_0.2s_ease-in-out]">
                          ...
                          <div className="w-full lg:w-1/3 flex flex-col gap-4 min-w-0">
                             ...
                             <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm w-full overflow-hidden">
                                ...
                                <div className="text-xs text-slate-600 leading-relaxed font-medium break-words whitespace-pre-wrap w-full">
                                  {lead.ai_summary ? (
                                    lead.ai_summary 
                                  ) : (
                          ...
                          <div className="w-full lg:w-2/3 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col h-[350px] min-w-0 max-w-full">
                             <div className="p-4 border-b border-slate-100 bg-slate-50/50 rounded-t-2xl flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 overflow-hidden w-full">
                               <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap flex-shrink-0">Chat History</h5>
                               {/* [FIX]: Tombol Sniper Search diizinkan wrap ke bawah kalau mentok (flex-wrap) */}
                               <div className="flex gap-2 flex-wrap w-full overflow-hidden">
                                 {lead.computedBadges.filter((b:any)=>b.value && b.value!=="null" && b.value!=="-").map((badge: any, idx: number) => (
                                    <button 
                                      ...
                                      className="text-[9px] bg-yellow-100 text-yellow-700 hover:bg-yellow-200 px-3 py-1.5 rounded-lg font-bold transition-all shadow-sm flex items-start gap-1.5 active:scale-95 border border-yellow-200 text-left max-w-full"
                                      title="Loncat ke pesan ini"
                                    >
                                        <span className="flex-shrink-0">🎯 <span className="uppercase opacity-70 hidden sm:inline">{badge.key}:</span></span> 
                                        <span className="inline-block break-words whitespace-normal leading-tight">{badge.value}</span>
                                    </button>
                                 ))}
                               </div>
                             </div>
                             {/* [FIX]: overflow-x-hidden pada area chat untuk potong paksa teks bubble yang keras kepala */}
                             <div id={`mini-chat-${lead.id}`} className="p-5 overflow-y-auto overflow-x-hidden flex-1 custom-scrollbar scroll-smooth w-full bg-slate-50/30">
